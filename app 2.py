...
║══════════════════════════════════════════════════════════════════════════════║
║  Developed by: Ali Riyad Faraj                                               ║
║  Location: Palestine 🇵🇸                                                      ║
║  Version: 2.0.0 (Medical Ecosystem Edition)                                  ║
║  Copyright © 2024-2025 Ali Riyad Faraj. All Rights Reserved.                 ║
║                                                                              ║
║  Description:                                                                ║
║  An intelligent system for food and medical analysis, built by Palestinian   ║
║  hands to empower community health awareness. Features include AI-powered    ║
║  food scanning, medical file vault, health dashboard, and personalized       ║
║  nutrition advice based on user medical profiles.                            ║
║                                                                              ║
║  نظام ذكي لتحليل المنتجات الغذائية والتقارير الطبية، صُمم بأيدي فلسطينية      ║
║  لتعزيز الوعي الصحي المجتمعي.                                                 ║
╚══════════════════════════════════════════════════════════════════════════════╝
"""
...
def init_database():
    ...
    cursor.execute('CREATE INDEX IF NOT EXISTS idx_user_id ON analysis_history(user_id)')
    cursor.execute('CREATE INDEX IF NOT EXISTS idx_medical_user_id ON medical_files(user_id)')
    cursor.execute('CREATE INDEX IF NOT EXISTS idx_medical_profile_user_id ON medical_profiles(user_id)')
...
# Version: 2.0.0 | Author: Ali Riyad Faraj | Palestine 🇵🇸
# ═══════════════════════════════════════════════════════════════════════════════
...
AI_OUTPUT_SCHEMA = {
    ...
    "decision_trace": ["string array - explainable AI decision path"],
    ...
    "medical_file_correlation": "string"
}

def build_analysis_prompt(language: str, medical_profile: Dict, history: List[Dict] = None, medical_files_summary: str = "") -> str:
    """
    Build production-grade AI analysis prompt with advanced features.
    Returns a comprehensive, evidence-based prompt for food analysis.
    
    Features:
    - Explainable Decision Trace
    - Long-term Impact Simulation
    - Nutrition Conflict Engine
    - Medical File Correlation
    """
    ...
    if medical_profile:
        ...
        if language == "ar":
            medical_context = f"""
═══════════════════════════════════════════════════════════════
⚠️ ملف المستخدم الطبي (إلزامي المراعاة):
- العمر: {age if age else 'غير محدد'}
- الوزن: {weight if weight else 'غير محدد'} كجم
- الطول: {height if height else 'غير محدد'} سم
- الحساسية: {allergies if allergies else 'لا يوجد'}
- الحالات الطبية: {conditions if conditions else 'لا يوجد'}

⚠️ تعليمات إلزامية:
- إذا كان أي مكون يتعارض مع حساسية المستخدم: أضفه في warnings مع علامة ⚠️
- إذا كان المنتج غير مناسب لحالة طبية (مثل السكري): اذكر ذلك في clinical_verdict
- خفض health_score بمقدار 20-40 نقطة إذا وُجد تعارض طبي
═══════════════════════════════════════════════════════════════"""
        elif language == "fr":
            medical_context = f"""
═══════════════════════════════════════════════════════════════
⚠️ PROFIL MÉDICAL UTILISATEUR (Obligatoire à considérer):
- Âge: {age if age else 'Non spécifié'}
- Poids: {weight if weight else 'Non spécifié'} kg
- Taille: {height if height else 'Non spécifié'} cm
- Allergies: {allergies if allergies else 'Aucune'}
- Conditions médicales: {conditions if conditions else 'Aucune'}

⚠️ Instructions obligatoires:
- Si un ingrédient entre en conflit avec les allergies: ajoutez-le aux warnings avec ⚠️
- Si le produit ne convient pas à une condition médicale (ex: diabète): mentionnez-le dans clinical_verdict
- Réduisez health_score de 20-40 points en cas de conflit médical
═══════════════════════════════════════════════════════════════"""
        else:
            medical_context = f"""
═══════════════════════════════════════════════════════════════
⚠️ USER MEDICAL PROFILE (Mandatory to consider):
...
- If ANY ingredient conflicts with user allergies: ADD to warnings with ⚠️ symbol
...
═══════════════════════════════════════════════════════════════"""
    ...
    if language == "ar":
        prompt = f"""أنت نظام تحليل غذائي طبي متقدم. مهمتك تحليل صورة المنتج الغذائي بدقة علمية.

═══════════════════════════════════════════════════════════════
📋 المهام المطلوبة:
═══════════════════════════════════════════════════════════════
1. استخراج اسم المنتج من الصورة
2. قراءة جدول القيم الغذائية بدقة (بروتين، كربوهيدرات، دهون، صوديوم)
3. استخراج قائمة المكونات كاملة
4. تحديد درجة NOVA للمعالجة (1-4):
   - NOVA 1: أغذية طبيعية غير معالجة
   - NOVA 2: مكونات طهي معالجة (زيوت، سكر، ملح)
   - NOVA 3: أغذية معالجة (خضار معلبة، جبن)
   - NOVA 4: منتجات فائقة المعالجة (وجبات سريعة، مشروبات غازية)

5. حساب نقاط الصحة (0-100) بناءً على:
   - محتوى السكر (كلما زاد، انخفضت النقاط)
   - محتوى الصوديوم (> 600mg يخفض النقاط)
   - الدهون المشبعة والمتحولة
   - درجة NOVA (NOVA 4 = خصم 20-30 نقطة)
   - وجود إضافات صناعية ومواد حافظة
   - تعارض مع الملف الطبي للمستخدم

6. تحديد الحكم بناءً على النقاط فقط:
   - 71-100: SAFE (آمن)
   - 41-70: WARNING (تحذير)
   - 0-40: DANGER (خطر)

7. كتابة تحليل سريري مفصل
8. إضافة تحذيرات محددة
9. اقتراح بديل صحي من نفس الفئة

═══════════════════════════════════════════════════════════════
🧠 ميزات الذكاء الاصطناعي المتقدمة (إلزامي):
═══════════════════════════════════════════════════════════════

10. تتبع القرارات الشفاف (Explainable Decision Trace):
    أنشئ مسار قرار شفاف يوضح لماذا تم الوصول للنتيجة.
    اذكر كل عامل أثر على القرار (المكونات، مستوى المعالجة، القيم الغذائية، الملف الطبي).
    كل عنصر في التتبع يجب أن يكون واقعياً ومحافظاً وغير تخميني.
    مثال: ["طعام فائق المعالجة (NOVA 4)", "صوديوم عالي (>600mg)", "المستخدم لديه ضغط دم", "تم خصم 25 نقطة من النقاط الصحية"]

11. محاكاة التأثير التراكمي على المدى الطويل:
    قدّر التأثير التراكمي المحتمل إذا تم استهلاك هذا المنتج بانتظام (مثلاً عدة مرات أسبوعياً على مدى شهر).
    ضع في الاعتبار أنماط استهلاك المستخدم السابقة إن كانت متاحة.
    قدم الاتجاه (إيجابي/محايد/سلبي)، ملخص، ومستوى الخطر (منخفض/متوسط/عالي).
    إذا كانت البيانات التاريخية غير كافية، اذكر ذلك بوضوح في الملخص.

12. محرك اكتشاف التعارضات الغذائية:
    اكتشف واذكر صراحة التعارضات بين:
    - المكونات وحساسيات المستخدم
    - القيم الغذائية والحالات الطبية للمستخدم
    - خصائص المنتج والأهداف الصحية
    كل تعارض يجب أن يتضمن: العامل، يتعارض_مع، والشدة (منخفض/متوسط/عالي).
    التعارضات يجب أن تكون مبنية على الأدلة ومحافظة.

13. ربط الملفات الطبية:
    إذا تم توفير ملخص الملفات الطبية أدناه، اربط تحليل الطعام مع تلك الملفات على مستوى عالٍ.
    ركز على الروابط ذات الصلة (مثلاً منتج عالي الصوديوم مع سجلات ضغط الدم، منتج عالي السكر مع السكري).
    قدم ارتباطاً موجزاً وذو صلة دون تشخيص أو نصيحة علاجية.
    إذا لم تكن هناك ملفات طبية متاحة، اذكر "لا توجد ملفات طبية متاحة".

{medical_context}
{f"═══════════════════════════════════════════════════════════════\n📁 ملخص الملفات الطبية للمستخدم:\n{medical_files_summary}\n═══════════════════════════════════════════════════════════════" if medical_files_summary else ""}
{f"═══════════════════════════════════════════════════════════════\n📊 سجل استهلاك المستخدم (آخر {len(history) if history else 0} فحص):\n{chr(10).join([f'- {item.get(\"product_name\", \"غير معروف\")}: النقاط {item.get(\"health_score\", \"غير متاح\")}, {item.get(\"verdict\", \"غير متاح\")}' for item in (history[:10] if history else [])])}\n═══════════════════════════════════════════════════════════════" if history and len(history) > 0 else ""}
═══════════════════════════════════════════════════════════════
...
{{
  ...
  "clinical_verdict": "التحليل السريري المفصل هنا...",
  "warnings": ["تحذير 1", "تحذير 2"],
  "bio_alternative": "البديل الصحي المقترح",
  "confidence_level": "high",
  "decision_trace": ["عامل 1", "عامل 2", "عامل 3"],
  "long_term_projection": {{
    "trend": "positive",
    "summary": "شرح موجز للتأثير التراكمي",
    "risk_level": "low"
  }},
  "nutrition_conflicts": [
    {{
      "factor": "صوديوم",
      "conflicts_with": "ضغط دم",
      "severity": "high"
    }}
  ],
  "medical_file_correlation": "ارتباط ذو صلة أو 'لا توجد ملفات طبية متاحة'"
}}

═══════════════════════════════════════════════════════════════
⚠️ قواعد صارمة:
═══════════════════════════════════════════════════════════════
- أخرج JSON فقط بدون أي نص أو شرح
- لا تختلق قيم - إذا لم تستطع القراءة اكتب "unknown"
- كن متحفظاً في التقييم (الأمان أولاً)
- إذا كانت جودة الصورة سيئة: confidence_level = "low"
- هذه أداة توعوية وليست تشخيصاً طبياً
- الحكم يُحدد من النقاط فقط (71+ آمن، 41-70 تحذير، 0-40 خطر)
"""

    elif language == "fr":
        prompt = f"""Vous êtes un système d'analyse nutritionnelle médicale avancé. Votre tâche est d'analyser l'image du produit alimentaire avec précision scientifique.

═══════════════════════════════════════════════════════════════
📋 TÂCHES REQUISES:
═══════════════════════════════════════════════════════════════
1. Extraire le nom du produit de l'image
2. Lire le tableau nutritionnel avec précision (protéines, glucides, lipides, sodium)
3. Extraire la liste complète des ingrédients
4. Déterminer le score NOVA (1-4):
   - NOVA 1: Aliments naturels non transformés
   - NOVA 2: Ingrédients culinaires (huiles, sucre, sel)
   - NOVA 3: Aliments transformés (légumes en conserve, fromage)
   - NOVA 4: Produits ultra-transformés (fast-food, sodas)

5. Calculer le score santé (0-100) basé sur:
   - Teneur en sucre (plus élevé = score plus bas)
   - Teneur en sodium (> 600mg réduit le score)
   - Graisses saturées et trans
   - Score NOVA (NOVA 4 = -20-30 points)
   - Présence d'additifs artificiels
   - Conflit avec le profil médical

6. Déterminer le verdict UNIQUEMENT selon le score:
   - 71-100: SAFE (Sûr)
   - 41-70: WARNING (Attention)
   - 0-40: DANGER (Danger)

7. Rédiger une analyse clinique détaillée
8. Ajouter des avertissements spécifiques
9. Suggérer une alternative plus saine

═══════════════════════════════════════════════════════════════
🧠 FONCTIONNALITÉS IA AVANCÉES (OBLIGATOIRE):
═══════════════════════════════════════════════════════════════

10. TRACE DE DÉCISION EXPLICABLE:
    Créez un chemin de décision transparent montrant POURQUOI le score et le verdict ont été atteints.
    Listez chaque facteur qui a influencé la décision (ingrédients, niveau de transformation, valeurs nutritionnelles, profil médical).
    Chaque élément de trace doit être factuel, conservateur et non spéculatif.
    Exemple: ["Aliment ultra-transformé (NOVA 4)", "Sodium élevé détecté (>600mg)", "L'utilisateur a de l'hypertension", "Score santé réduit de 25 points"]

11. SIMULATION D'IMPACT À LONG TERME:
    Estimez l'impact cumulatif potentiel si ce produit est consommé régulièrement (ex: plusieurs fois par semaine sur un mois).
    Considérez les modèles de consommation historique de l'utilisateur si disponibles.
    Fournissez la tendance (positive/neutre/négative), résumé, et niveau de risque (faible/modéré/élevé).
    Si les données historiques sont insuffisantes, indiquez-le clairement dans le résumé.

12. MOTEUR DE CONFLITS NUTRITIONNELS:
    Détectez et listez explicitement les conflits entre:
    - Ingrédients et allergies de l'utilisateur
    - Valeurs nutritionnelles et conditions médicales de l'utilisateur
    - Caractéristiques du produit et objectifs de santé
    Chaque conflit doit inclure: facteur, conflicts_with, et sévérité (faible/modéré/élevé).
    Les conflits doivent être basés sur des preuves et prudents.

13. CORRÉLATION DES DOSSIERS MÉDICAUX:
    Si un résumé des dossiers médicaux est fourni ci-dessous, corrélez l'analyse alimentaire avec ces dossiers à un niveau élevé.
    Concentrez-vous sur les connexions pertinentes (ex: produit riche en sodium avec antécédents d'hypertension, produit riche en sucre avec diabète).
    Fournissez une corrélation brève et pertinente sans diagnostic ni conseil de traitement.
    Si aucun dossier médical n'est disponible, indiquez "aucun dossier médical disponible".

{medical_context}
{f"═══════════════════════════════════════════════════════════════\n📁 RÉSUMÉ DES DOSSIERS MÉDICAUX DE L'UTILISATEUR:\n{medical_files_summary}\n═══════════════════════════════════════════════════════════════" if medical_files_summary else ""}
{f"═══════════════════════════════════════════════════════════════\n📊 HISTORIQUE DE CONSOMMATION DE L'UTILISATEUR (Derniers {len(history) if history else 0} scans):\n{chr(10).join([f'- {item.get(\"product_name\", \"Inconnu\")}: Score {item.get(\"health_score\", \"N/A\")}, {item.get(\"verdict\", \"N/A\")}' for item in (history[:10] if history else [])])}\n═══════════════════════════════════════════════════════════════" if history and len(history) > 0 else ""}
═══════════════════════════════════════════════════════════════
📤 FORMAT DE SORTIE (JSON uniquement - aucun texte supplémentaire):
═══════════════════════════════════════════════════════════════
{{
  "name": "Nom du produit",
  "health_score": 0,
  ...
  "clinical_verdict": "Analyse clinique détaillée ici...",
  "warnings": ["avertissement 1", "avertissement 2"],
  "bio_alternative": "Alternative saine suggérée",
  "confidence_level": "high",
  "decision_trace": ["Facteur 1", "Facteur 2", "Facteur 3"],
  "long_term_projection": {{
    "trend": "positive",
    "summary": "Brève explication de l'impact cumulatif",
    "risk_level": "low"
  }},
  "nutrition_conflicts": [
    {{
      "factor": "sodium",
      "conflicts_with": "hypertension",
      "severity": "high"
    }}
  ],
  "medical_file_correlation": "Corrélation pertinente ou 'aucun dossier médical disponible'"
}}
...
- Ne pas inventer de valeurs - écrire "unknown" si illisible
- Être conservateur dans l'évaluation (sécurité d'abord)
- Si qualité d'image mauvaise: confidence_level = "low"
- Ceci est un outil de sensibilisation, PAS un diagnostic médical
- Le verdict est déterminé UNIQUEMENT par le score (71+ sûr, 41-70 attention, 0-40 danger)
"""
...
═══════════════════════════════════════════════════════════════
🧠 ADVANCED AI FEATURES (MANDATORY):
═══════════════════════════════════════════════════════════════

10. EXPLAINABLE DECISION TRACE:
    Create a transparent decision path showing WHY the score and verdict were reached.
    List each factor that influenced the decision (ingredients, processing level, nutrition values, medical profile).
    Each trace item must be factual, conservative, and non-speculative.
    Example: ["Ultra-processed food (NOVA 4)", "High sodium detected (>600mg)", "User has hypertension", "Health score reduced by 25 points"]

11. LONG-TERM IMPACT SIMULATION:
    Estimate potential cumulative impact if this product is consumed regularly (e.g., several times per week over a month).
    Consider user's consumption history patterns if available.
    Provide trend (positive/neutral/negative), summary, and risk_level (low/moderate/high).
    If historical data is insufficient, state that clearly in the summary.

12. NUTRITION CONFLICT ENGINE:
    Explicitly detect and list conflicts between:
    - Ingredients and user allergies
    - Nutrition values and user medical conditions
    - Product characteristics and health goals
    Each conflict must include: factor, conflicts_with, and severity (low/moderate/high).
    Conflicts must be evidence-based and cautious.

13. MEDICAL FILE CORRELATION:
    If medical files summary is provided below, correlate the food analysis with those files at a high level.
    Focus on relevant connections (e.g., high sodium product with hypertension records, high sugar with diabetes).
    Provide a brief, relevant correlation without diagnosis or treatment advice.
    If no medical files available, state "no medical files available".

{medical_context}
{f"═══════════════════════════════════════════════════════════════\n📁 USER MEDICAL FILES SUMMARY:\n{medical_files_summary}\n═══════════════════════════════════════════════════════════════" if medical_files_summary else ""}
{f"═══════════════════════════════════════════════════════════════\n📊 USER CONSUMPTION HISTORY (Last {len(history) if history else 0} scans):\n{chr(10).join([f'- {item.get(\"product_name\", \"Unknown\")}: Score {item.get(\"health_score\", \"N/A\")}, {item.get(\"verdict\", \"N/A\")}' for item in (history[:10] if history else [])])}\n═══════════════════════════════════════════════════════════════" if history and len(history) > 0 else ""}
═══════════════════════════════════════════════════════════════
...
{{
  ...
  "confidence_level": "high",
  "decision_trace": ["Factor 1", "Factor 2", "Factor 3"],
  "long_term_projection": {{
    "trend": "positive",
    "summary": "Brief explanation of cumulative impact",
    "risk_level": "low"
  }},
  "nutrition_conflicts": [
    {{
  ...
  "medical_file_correlation": "Relevant correlation or 'no medical files available'"
}}
...
def validate_ai_response(response: Dict) -> Dict:
    ...
    validated = {
        ...
        "long_term_projection": {
            "trend": "neutral",
            "summary": "Insufficient data for long-term projection",
            "risk_level": "low"
        },
        "nutrition_conflicts": [],
        "medical_file_correlation": "no medical files available"
    }
    ...
    # Validate decision_trace
    if isinstance(response.get("decision_trace"), list):
        validated["decision_trace"] = [str(t) for t in response["decision_trace"] if t]
    else:
        validated["decision_trace"] = []
    
    # Validate long_term_projection
    if isinstance(response.get("long_term_projection"), dict):
        proj = response["long_term_projection"]
        if proj.get("trend") in ["positive", "neutral", "negative"]:
            validated["long_term_projection"]["trend"] = proj["trend"]
        if isinstance(proj.get("summary"), str):
            validated["long_term_projection"]["summary"] = proj["summary"]
        if proj.get("risk_level") in ["low", "moderate", "high"]:
            validated["long_term_projection"]["risk_level"] = proj["risk_level"]
    
    # Validate nutrition_conflicts
    if isinstance(response.get("nutrition_conflicts"), list):
        conflicts = []
        for conflict in response["nutrition_conflicts"]:
            if isinstance(conflict, dict):
                valid_conflict = {
                    "factor": str(conflict.get("factor", "unknown")),
                    "conflicts_with": str(conflict.get("conflicts_with", "unknown")),
                    "severity": conflict.get("severity", "low") if conflict.get("severity") in ["low", "moderate", "high"] else "low"
                }
                conflicts.append(valid_conflict)
        validated["nutrition_conflicts"] = conflicts
    
    # Validate medical_file_correlation
    if isinstance(response.get("medical_file_correlation"), str):
        validated["medical_file_correlation"] = response["medical_file_correlation"]
    
    return validated

def analyze_food_product(client, image: Image.Image, language: str, medical_profile: Dict, history: List[Dict] = None, medical_files_summary: str = "") -> Dict:
    """
    Main AI analysis function for food products with advanced features.
    Returns validated JSON response with:
    - Explainable Decision Trace
    - Long-term Impact Simulation
    - Nutrition Conflict Engine
    - Medical File Correlation
    ...
    Args:
        ...
        image: Product image
        language: User language (en/ar/fr)
        medical_profile: User medical profile dict
        history: User's consumption history (optional)
        medical_files_summary: Summary of user's medical files (optional)
    """
    try:
        # Build comprehensive prompt with advanced features
        prompt = build_analysis_prompt(language, medical_profile, history, medical_files_summary)
        ...
        try:
            ...
            return {
                ...
                "ingredients": ["unknown"],
                "clinical_verdict": "Analysis could not be completed. Please try again with a clearer image.",
                "warnings": ["Unable to parse AI response"],
                "bio_alternative": "N/A",
                "confidence_level": "low",
                "decision_trace": ["JSON parsing failed"],
                "long_term_projection": {"trend": "neutral", "summary": "Analysis incomplete", "risk_level": "low"},
                "nutrition_conflicts": [],
                "medical_file_correlation": "no medical files available",
                "_raw_response": response_text[:500]  # For debugging
            }
        ...
        return {
            ...
            "macros": {"protein": "unknown", "carbs": "unknown", "fats": "unknown", "sodium": "unknown"},
            "ingredients": ["unknown"],
            "clinical_verdict": f"An error occurred during analysis. Please try again.",
            "warnings": ["Analysis failed"],
            "bio_alternative": "N/A",
            "confidence_level": "low",
            "decision_trace": ["Analysis error occurred"],
            "long_term_projection": {"trend": "neutral", "summary": "Analysis incomplete", "risk_level": "low"},
            "nutrition_conflicts": [],
            "medical_file_correlation": "no medical files available"
        }
...
TRANSLATIONS = {
    "en": {
        ...
        "api_missing": "⚠️ API key not found. Please add OPENAI_API_KEY",
        "analyzing": "Analyzing...",
        ...
        "features": "✨ Features",
        "features_list": "• Instant food product analysis<br>• Comprehensive health score (0-100)<br>• NOVA classification for processed foods<br>• Healthy alternatives suggestions<br>• Previous analyses history",
        ...
        "footer_disclaimer": "⚕️ BioGuard AI is an awareness tool only and does not constitute medical diagnosis. Always consult a healthcare professional.",
        "personalized_note": "📋 Personalized analysis based on your medical profile",
        "medical_vault": "🗂️ Medical Vault",
        ...
        "sync_records": "🔄 Sync Medical Records",
        ...
        "developer_info": "👨‍💻 Developer: Ali Riyad Faraj",
        "location_info": "📍 Location: Palestine 🇵🇸",
        "version_info": "📦 Version: 2.0.0",
        "made_in_palestine": "Made in Palestine 🇵🇸",
    ...
    "ar": {
        "app_title": "BioGuard AI",
        "app_subtitle": "حارسك الصحي الذكي",
        "change_theme": "🔄 تغيير السمة",
        "scan_product": "امسح منتجك الغذائي",
        "upload_hint": "ارفع صورة الباركود أو جدول المكونات",
        "choose_image": "اختر صورة",
        "analyze_btn": "🔍 تحليل المنتج",
        "api_missing": "⚠️ مفتاح API غير موجود. يرجى إضافة OPENAI_API_KEY",
        "analyzing": "جاري التحليل...",
        "analysis_failed": "فشل التحليل. تأكد من وضوح الصورة وحاول مرة أخرى.",
        "health_score": "نقطة صحية",
        "safe": "آمن ✓",
        "warning": "تحذير ⚡",
        "danger": "خطر ⚠",
        "protein": "بروتين",
        ...
        "fats": "دهون",
        "sodium": "صوديوم",
        "clinical_analysis": "🩺 التحليل الطبي",
        "healthy_alternative": "✨ البديل الصحي",
        "history": "📋 السجل",
        "no_history": "لا توجد تحليلات سابقة",
        "about": "حول",
        "about_desc": "نظام ذكي لتحليل المنتجات الغذائية باستخدام الذكاء الاصطناعي. يساعدك على اتخاذ قرارات صحية أفضل من خلال تحليل المكونات والقيم الغذائية.",
        "features": "✨ المميزات",
        "features_list": "• تحليل فوري للمنتجات الغذائية<br>• تقييم صحي شامل (0-100)<br>• تصنيف NOVA للأغذية المصنعة<br>• اقتراح بدائل صحية<br>• سجل التحليلات السابقة",
        "disclaimer": "⚠️ تنبيه",
        "disclaimer_text": "هذا التطبيق للتوعية الصحية فقط وليس بديلاً عن الاستشارة الطبية المتخصصة.",
        "nav_scan": "📸 مسح",
        "nav_history": "📋 السجل",
        "nav_about": "ℹ️ حول",
        "nav_chat": "💬 دردشة",
        ...
        "medical_history_desc": "يرجى ملء معلوماتك الطبية للحصول على تحليل مخصص",
        "age": "العمر",
        "gender": "الجنس",
        "male": "ذكر",
        "female": "أنثى",
        ...
        "height": "الطول (سم)",
        "allergies": "الحساسية",
        "allergies_hint": "مثال: فول سوداني، ألبان، جلوتين",
        "conditions": "الحالات الطبية",
        "conditions_hint": "مثال: سكري، ضغط دم",
        "save_profile": "💾 حفظ الملف الشخصي",
        "profile_saved": "تم حفظ الملف الشخصي بنجاح!",
        "chat_title": "🍎 الدردشة الغذائية",
        "chat_subtitle": "اسألني أي شيء عن التغذية والصحة",
        ...
        "footer_disclaimer": "⚕️ BioGuard AI أداة توعوية فقط ولا تشكل تشخيصاً طبياً. استشر دائماً أخصائي رعاية صحية.",
        "personalized_note": "📋 تحليل مخصص بناءً على ملفك الطبي",
        "medical_vault": "🗂️ الخزنة الطبية",
        "medical_vault_desc": "احفظ وأدر وثائقك الطبية",
        "upload_medical_file": "رفع ملف طبي",
        "file_category": "نوع الملف",
        "cat_xray": "أشعة / فحص",
        "cat_lab": "نتائج مختبر",
        "cat_prescription": "وصفة طبية",
        "cat_report": "تقرير طبي",
        "cat_other": "أخرى",
        "analyze_file": "🔍 تحليل بالذكاء الاصطناعي",
        "your_files": "ملفاتك الطبية",
        "no_files": "لم يتم رفع أي ملفات طبية بعد",
        "delete_file": "حذف",
        "dashboard": "📊 لوحة التحكم",
        "avg_health_score": "متوسط النقاط الصحية",
        "total_scans": "إجمالي الفحوصات",
        "safe_products": "منتجات آمنة",
        "warning_products": "منتجات تحذيرية",
        "danger_products": "منتجات خطرة",
        "nutrition_summary": "ملخص التغذية",
        "total_carbs": "إجمالي الكربوهيدرات",
        "total_fats": "إجمالي الدهون",
        "total_sodium": "إجمالي الصوديوم",
        "recent_scans": "الفحوصات الأخيرة",
        "sync_records": "🔄 مزامنة السجلات الطبية",
        "sync_coming_soon": "المزامنة الخارجية قريباً!",
        "nav_vault": "🗂️ الخزنة",
        "about_desc": "نظام ذكي لتحليل المنتجات الغذائية والتقارير الطبية، صُمم بأيدي فلسطينية لتعزيز الوعي الصحي المجتمعي.",
        "developer_info": "👨‍💻 مطور التطبيق: علي رياض فرج",
        "location_info": "📍 الموقع: فلسطين 🇵🇸",
        "version_info": "📦 الإصدار: 2.0.0",
        "made_in_palestine": "صنع في فلسطين 🇵🇸",
        "privacy_title": "🔒 الخصوصية والأمان",
        "privacy_text": "نؤكد أن كافة بيانات المستخدمين والتقارير الطبية المرفوعة تُعالج بمنتهى الخصوصية، مع الالتزام بعدم مشاركة أي بيانات حساسة.",
        "disclaimer_full": "هذا التطبيق (BioGuard AI) هو جهد تقني من المبرمج علي رياض فرج، وهو مخصص للأغراض التعليمية والتوعوية فقط. نظراً لخصوصية الحالة الصحية في فلسطين، يُنصح دائماً بمراجعة المراكز الطبية الفلسطينية المعتمدة قبل اتخاذ أي قرار طبي بناءً على تحليلات الذكاء الاصطناعي."
    },
    "fr": {
        "app_title": "BioGuard AI",
        "app_subtitle": "Votre Gardien Santé Intelligent",
        "change_theme": "🔄 Changer le Thème",
        "scan_product": "Scannez Votre Produit",
        "upload_hint": "Téléchargez l'image du code-barres ou des ingrédients",
        "choose_image": "Choisir une image",
        "analyze_btn": "🔍 Analyser le Produit",
        "api_missing": "⚠️ Clé API introuvable. Veuillez ajouter OPENAI_API_KEY",
        "analyzing": "Analyse en cours...",
        "analysis_failed": "Échec de l'analyse. Assurez-vous que l'image est claire et réessayez.",
        ...
        "healthy_alternative": "✨ Alternative Saine",
        "history": "📋 Historique",
        "no_history": "Aucune analyse précédente",
        "about": "À Propos",
        "about_desc": "Un système intelligent d'analyse des produits alimentaires utilisant l'IA. Vous aide à prendre de meilleures décisions santé en analysant les ingrédients et les valeurs nutritionnelles.",
        "features": "✨ Fonctionnalités",
        "features_list": "• Analyse instantanée des produits<br>• Score santé complet (0-100)<br>• Classification NOVA des aliments transformés<br>• Suggestions d'alternatives saines<br>• Historique des analyses",
        "disclaimer": "⚠️ Avertissement",
        "disclaimer_text": "Cette application est uniquement à des fins de sensibilisation à la santé et ne remplace pas un avis médical professionnel.",
        "nav_scan": "📸 Scanner",
        ...
        "medical_history_desc": "Veuillez remplir vos informations médicales pour une analyse personnalisée",
        "age": "Âge",
        "gender": "Genre",
        "male": "Homme",
        "female": "Femme",
        ...
        "conditions_hint": "ex: diabète, hypertension",
        "save_profile": "💾 Enregistrer le Profil",
        "profile_saved": "Profil enregistré avec succès!",
        "chat_title": "🍎 Chat Nutrition",
        "chat_subtitle": "Posez-moi vos questions sur la nutrition et la santé",
        ...
        "footer_disclaimer": "⚕️ BioGuard AI est un outil de sensibilisation uniquement et ne constitue pas un diagnostic médical. Consultez toujours un professionnel de santé.",
        "personalized_note": "📋 Analyse personnalisée basée sur votre profil médical",
        "medical_vault": "🗂️ Coffre Médical",
        "medical_vault_desc": "Stockez et gérez vos documents médicaux",
        "upload_medical_file": "Télécharger un fichier médical",
        "file_category": "Catégorie du fichier",
        "cat_xray": "Radiographie / Scan",
        "cat_lab": "Résultats de laboratoire",
        "cat_prescription": "Ordonnance",
        "cat_report": "Rapport médical",
        "cat_other": "Autre",
        "analyze_file": "🔍 Analyser avec l'IA",
        "your_files": "Vos fichiers médicaux",
        "no_files": "Aucun fichier médical téléchargé",
        "delete_file": "Supprimer",
        "dashboard": "📊 Tableau de bord",
        "avg_health_score": "Score santé moyen",
        "total_scans": "Total des scans",
        "safe_products": "Produits sûrs",
        "warning_products": "Produits à surveiller",
        ...
        "sync_records": "🔄 Synchroniser les dossiers médicaux",
        "sync_coming_soon": "Synchronisation externe bientôt disponible!",
        "nav_vault": "🗂️ Coffre",
        "about_desc": "Un système intelligent d'analyse alimentaire et médicale, créé par des mains palestiniennes pour promouvoir la sensibilisation à la santé communautaire.",
        "developer_info": "👨‍💻 Développeur: Ali Riyad Faraj",
        "location_info": "📍 Lieu: Palestine 🇵🇸",
        "version_info": "📦 Version: 2.0.0",
        "made_in_palestine": "Fabriqué en Palestine 🇵🇸",
        "privacy_title": "🔒 Confidentialité et Sécurité",
        "privacy_text": "Toutes les données utilisateur et rapports médicaux sont traités avec la plus grande confidentialité. Nous nous engageons à ne partager aucune donnée sensible.",
        "disclaimer_full": "Cette application (BioGuard AI) est un effort technique du développeur Ali Riyad Faraj, destinée uniquement à des fins éducatives et de sensibilisation. Il est toujours conseillé de consulter des centres médicaux certifiés avant de prendre toute décision médicale basée sur l'analyse IA."
    }
}
...
📦 Product: {result.get('name', 'Unknown')}
📊 Health Score: {result.get('health_score', 0)}/100
🏷️ NOVA Score: {result.get('nova_score', 'N/A')}
...
─────────────────────
  • Age: {medical_profile.get('age', 'N/A')}
  • Allergies: {medical_profile.get('allergies', 'None')}
  • Conditions: {medical_profile.get('conditions', 'None')}
"""
    
    report += """
═══════════════════════════════════════
⚕️ DISCLAIMER: This is an awareness tool
...
def render_header():
    ...
    <div class="mobile-header">
        <div class="app-logo" style="position: relative;">
            ...
            <span style="position: absolute; bottom: -5px; right: -8px; font-size: 1.2rem;">🩺</span>
        </div>
        ...
        <div style="font-size: 0.65rem; opacity: 0.6; margin-top: 0.2rem;">🇵🇸 {t('made_in_palestine')}</div>
    </div>
    """, unsafe_allow_html=True)
        ...
        if st.button(t('analyze_btn'), key="analyze_btn"):
            ...
            with st.spinner(t('analyzing')):
                try:
                    ...
                    # Get consumption history for long-term projection
                    history = []
                    if user_id:
                        history = db_get_history(user_id, limit=20)
                    
                    # Get medical files summary for correlation
                    medical_files_summary = ""
                    if user_id:
                        medical_files_summary = db_get_user_files_summary(user_id)
                    ...
                    result = analyze_food_product(
                        ...
                        medical_profile,
                        history=history,
                        medical_files_summary=medical_files_summary
                    )
                    ...
                    if confidence == "low":
                        st.warning("⚠️ Low confidence - image quality may affect accuracy. Consider retaking the photo.")
                    ...
                    if "401" in error_msg or "API_KEY" in error_msg.upper() or "invalid" in error_msg.lower():
                        st.warning("🔑 API Key issue - check your OPENAI_API_KEY in .env file")
                    elif "safety" in error_msg.lower() or "blocked" in error_msg.lower():
                        st.warning("🛡️ Content blocked by safety filters - try a different image")
...
def render_result(result: Dict[str, Any]):
    ...
    if ingredients and isinstance(ingredients, list) and ingredients[0] != "unknown":
        ...
        st.markdown(f"""
        <div class="glass-card" style="padding: 0.8rem;">
            <div style="font-size: 0.8rem; font-weight: 600; color: {theme['primary']}; margin-bottom: 0.5rem;">📋 Ingredients</div>
    ...
    if warnings and isinstance(warnings, list):
        for w in warnings:
            ...
            <div class="info-box" style="border-left-color: #ff4444;">
                <div class="info-content" style="color: #ff4444;">⚠️ {w}</div>
            </div>
            """, unsafe_allow_html=True)
            ...
            <div class="glass-card">
                <div style="display: flex; justify-content: space-between; align-items: center;">
                    ...
                    <div style="font-size: 1.5rem; color: {color};">
                        {"✓" if verdict == "SAFE" else "⚠" if verdict == "DANGER" else "⚡"}
                    </div>
                </div>
            </div>
            """, unsafe_allow_html=True)
...
def render_about_view():
    ...
    <div style='text-align: center; padding: 15px; margin-bottom: 1rem; border-radius: 15px; 
        ...
        <h4 style='color: #ffffff; text-shadow: 2px 2px 4px #000000; margin: 0;'>🇵🇸</h4>
    </div>
    """, unsafe_allow_html=True)
    ...
    <div class="glass-card" style="text-align: center;">
        <div style="font-size: 3rem; margin-bottom: 0.5rem; position: relative; display: inline-block;">
            ...
            <span style="position: absolute; bottom: -5px; right: -12px; font-size: 1.5rem;">🩺</span>
        </div>
    ...
    <div style="text-align: center; padding: 1rem; opacity: 0.7; font-size: 0.75rem;">
        © 2024-2025 Ali Riyad Faraj. All Rights Reserved.
    </div>
    """, unsafe_allow_html=True)
        ...
        if user_input:
            ...
            if client:
                try:
                    ...
                    lang = st.session_state.language
                    if lang == "ar":
                        system_msg = "أنت خبير تغذية ودكتور صحة متخصص. لديك إمكانية الوصول لملخص الملفات الطبية للمستخدم. أجب على أسئلة المستخدم بشكل مفيد ومختصر مع مراعاة حالته الصحية. " + medical_ctx
                    elif lang == "fr":
                        system_msg = "Vous êtes un expert en nutrition et santé. Vous avez accès au résumé des dossiers médicaux de l'utilisateur. Répondez de manière utile et concise en tenant compte de son état de santé. " + medical_ctx
                    else:
...
def render_medical_vault_view():
        ...
        with col1:
            if st.button(t('save_profile'), key="save_file_btn", use_container_width=True):
                if st.session_state.get('user_id'):
                    ...
                    st.success(f"✅ File saved! ID: {file_id}")
                    st.rerun()
    ...
    if st.session_state.get('user_id'):
        ...
        if files:
            ...
            for f in files:
                icon = "📄" if "pdf" in f['file_type'] else "🖼️"
                ...
                <div class="glass-card" style="padding: 0.8rem;">
                    <div style="display: flex; justify-content: space-between; align-items: center;">
                        <div>
                            ...
                            <div style="font-size: 0.75rem; opacity: 0.7;">{cat_display} • {f['created_at'][:10]}</div>
...
def render_user_header():
    ...
    if st.session_state.logged_in:
        ...
        with col1:
            st.markdown(f"<p style='color: {theme['primary']};'>👤 {t('welcome')}, {st.session_state.username}!</p>", unsafe_allow_html=True)
        with col2:
...
def render_footer():
    ...
    <div class="footer-disclaimer">
        ...
        <div style="opacity: 0.7; font-size: 0.6rem;">
            © 2024-2025 Ali Riyad Faraj | 🇵🇸 Palestine | v{__version__}
        </div>
    </div>
    """, unsafe_allow_html=True)
