...
def _handle_capture(processor: BioGuardVideoProcessor, messages: Dict[str, str]) -> None:
    ...
    with st.spinner(messages['analyzing']):
        ...
        try:
            ...
            if st.session_state.get('authenticated'):
                db = get_db_manager()
                db.save_analysis(
                    user_id=st.session_state.get('user_profile', {}).get('email', 'guest'),
...
def _display_detection_info(result: Dict[str, Any], messages: Dict[str, str]) -> None:
    ...
    if detections:
        ...
        with st.expander(f"{messages['detections_found']}: {len(detections)}", expanded=False):
...
def _display_barcode_info(result: Dict[str, Any], messages: Dict[str, str]) -> None:
    ...
    if barcode and barcode != st.session_state.last_barcode:
        ...
        with st.expander(f"📊 {messages['barcode_detected']}: {barcode}", expanded=True):
            ...
            if product_info:
                ...
                with col2:
                    st.markdown(f"**{messages['product_name']}:** {product_info.get('name', 'N/A')}")
                    st.markdown(f"**{messages['brand']}:** {product_info.get('brands', 'N/A')}")
...
def _render_upload_interface(messages: Dict[str, str]) -> None:
    ...
    if uploaded_file:
        ...
        if st.button(messages['analyze_button'], type='primary', use_container_width=True):
            with st.spinner(messages['analyzing']):
                try:
                    # Convert to bytes
                    img_byte_arr = BytesIO()
